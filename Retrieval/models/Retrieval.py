...
from layers.Transformer_Enc import Transformer_Enc
...
class Model(nn.Module):
    ...
    def forward(self, x, use_mask=False):           # x: [Batch, Input length, Channel]
        x = x.permute(0,2,1)    # x: [Batch, Channel, Input length]
        if use_mask:
            z, z_hat = self.model(x, use_mask)
            return z, z_hat
        else:
            cls_token = self.model(x, use_mask)
            return cls_token