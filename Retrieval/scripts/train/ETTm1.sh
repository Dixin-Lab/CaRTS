...
if [ ! -d "./logs/ETTm1_ETTm2" ]; then
    mkdir ./logs/ETTm1_ETTm2
fi
...
python -u -m torch.distributed.run --nproc_per_node=6 --master_port=2424 run.py \
    ...
    --normalization True \
    --use_multi_gpu \
    --itr 1 --batch_size 128 --learning_rate 0.001 >logs/ETTm1_ETTm2/$model_name'_'$model_id_name'_'$retrieval_data'_'$seq_len'_'$pred_len'.log' 
done
